...
import { getChapitres, deleteChapitre } from '../../../../../api/chapitre';
...
const Chapitre = () => {
  ...
  const handleDelete = async (uid) => {
    ...
    if (result.isConfirmed) {
      setLoading(true);
      try {
        await deleteChapitre(uid);
        fetchChapitres(); // Rafraîchir la liste après suppression
        Swal.fire('Succès !', 'Chapitre supprimé avec succès.', 'success');
      } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-gray-800">Gestion des chapitres</h2>
        <button
          onClick={handleAdd}
      ...
      {!loading && chapitres && chapitres.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          Aucun chapitre trouvé. Cliquez sur "Ajouter un chapitre" pour en créer un.
        </div>
      ) : (
                ...
                <tr key={chapitre.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex space-x-3">
                      ...
                      <button
                        onClick={() => handleDelete(chapitre.uid)}
