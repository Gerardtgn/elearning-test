...
export const LevelForm = ({ level, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        // Mode édition
        await updateLevel(level.uid, formData);
      } else {
        // Mode ajout
      ...
      setError(
        ...
        `Erreur lors de ${isEditMode ? 'la modification' : 'l\'ajout'} du niveau`
      );
      console.error(`Erreur lors de ${isEditMode ? 'la modification' : 'l\'ajout'} du niveau:`, err);
       Swal.fire(
                  'Erreur !',
                  `Erreur lors de ${isEditMode ? 'la modification' : 'l\'ajout'} du niveau:`, err,
                  'error'
              );
    } finally {
      setLoading(false);
